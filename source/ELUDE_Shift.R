...
rtPeptide <- rtPeptide[(mod1=="M<Mox*>" | mod1=="M<Mox>" | mod1=="" | mod1=="C<Cmm*>" | mod1=="K<c13>" | mod1=="R<C13>" | mod1=="Q<Pyr>") &
                         modN=="NH2-"]
...
rtPeptide[, elude_sequence := gsub("Mox\\*","15,994915", elude_sequence)]
rtPeptide[, elude_sequence := gsub("Mox","15,994915", elude_sequence)]
rtPeptide[, elude_sequence := gsub("Cmm\\*","57,021464", elude_sequence)]
rtPeptide[, elude_sequence := gsub("Pyr","-17", elude_sequence)]
...
minTrain = min(rtPeptideTrain[, q50_5])
maxTrain = max(rtPeptideTrain[, q50_5])
subFactor = mean(c(minTrain, maxTrain))
divFactor = maxTrain - subFactor
rtPeptideTrain[, q50_5 := (q50_5 - subFactor) / divFactor]
...
rtPeptideTest[, corrected_RT := (corrected_RT - subFactor) / divFactor]
...
rtPeptideTestMedian[, q50_5 := (q50_5 - subFactor) / divFactor]
write.table(rtPeptideTestMedian, file=paste0(projectPath, "/data/rtPeptideTestMedian.txt"), quote = FALSE, , sep="\t", row.names = FALSE, col.names = FALSE)
# Let's assess the performance on 
testData <- "/data/rtPeptideTestMedian.txt"
testData <- shQuote(paste0(projectPath, testData))

savePredict <- "/data/predictionsTest.out"
savePredict <- shQuote(paste0(projectPath, savePredict))
...
results <- data.table(read.table(file=paste0(projectPath, "/data/predictionsTest.out"), header = TRUE, sep = "\t"))
...
results[, q975 := quantile(diff, probs = 0.975), by = centile]
results[, q025 := quantile(diff, probs = 0.025), by = centile]
results[, q500 := quantile(diff, probs = 0.500), by = centile]
results[, q250 := quantile(diff, probs = 0.250), by = centile]
results[, q750 := quantile(diff, probs = 0.750), by = centile]


ggplot(results, aes(Observed_RT, meanError)) + geom_point(alpha=(1/2))
ggplot(results, aes(centile, q500)) + geom_point(alpha=(1/2))
ggplot(results, aes(centile, q90_pred)) + geom_point(alpha=(1/2))


# Very long to compute, ok with testmedian, not ok with test
...
ggplot(graphDS, aes(Observed_RT, error, colour = quantile)) + geom_point(alpha=(1/2)) + xlim(-1,1)+ ylim(-0.6,0.3)

save(graphDS, file = paste0(projectPath,"/plot/graphCorrected.RData"), compression_level = 1)
